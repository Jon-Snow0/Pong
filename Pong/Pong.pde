...
AudioSample fatality;//sound after every point
AudioSample finishHim;//spund at 6 points
void setup() {
  frameRate(60);
  size(displayWidth, displayHeight);
  minim=new Minim(this);
  player=minim.loadFile("Mortal Kombat SONG.mp3");
  fatality=minim.loadSample("Fatality (Mortal Kombat) Sound Effect.mp3");
  finishHim=minim.loadSample("Mortal Kombat's Finish Him sound byte.mp3");
  keys=new boolean[4]; 
  keys[0]=false;
  keys[1]=false;
  keys[2]=false;
  ...
  ballx=width/2;
  bally=height/2;
  ball=loadImage("Pong Ball.png");
  background=loadImage("pong background.jpg");
}
